...
import {
  Formatter,
  Date as DatePicker
} from "../../components/Tools/Formatter";
    ...
    <Form.MainLabelContainer {...props}>
      <Form.AltBox>
        ...
        <Form.AltBox>
          ...
          <Form.Limit>{media}</Form.Limit>
        </Form.AltBox>
      </Form.AltBox>
    </Form.MainLabelContainer>
  );
};
export const MainPhoneFormatter = ({
  name,
  label,
  placeholder,
  type,
...
export const MainUpload = ({ name, label, placeholder, ...props }) => {
  return (
    <Form.MainLabelContainer htmlFor="partner-attachment" {...props}>
      ...
      <Form.AltBox>
        <div>
          <Form.MainInputDisabled width={props.width} height={props.height}>
            {placeholder}
          </Form.MainInputDisabled>
          <Image
            src={require("../../assets/images/attachment_file.svg")}
            width="32px"
            height="32px"
            margin="0 8px"
            cursor="pointer"
    ...
    <Form.MainLabelContainer htmlFor="partner-attachment" {...props}>
      <Form.AltBox>
        ...
        <Form.AltBox>
          ...
          <Form.Limit>Upload file (Not more than 2mb)</Form.Limit>
          <Form.MainInput
            type="file"
            id="partner-attachment"
            name={name}
...
export const Select = ({ name, label, children, value, changed, ...props }) => {
  return (
    <Form.MainLabelContainer maxWidth={props.maxWidth}>
      <Form.MainLabel>
        <Type.BoldLabel {...props}> {label} </Type.BoldLabel>
      </Form.MainLabel>
      <Form.Select name={name} value={value} onChange={changed} {...props}>
        {" "}
        {children}{" "}
      </Form.Select>
    </Form.MainLabelContainer>
...
export const CheckBox = ({ label, name, type, checkmarkClass, ...props }) => {
  return (
    <Form.CheckBoxLabel zIndex={props.zIndex} className={props.labelClass}>
      <Form.MainLabel>
