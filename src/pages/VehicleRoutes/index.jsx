...
class index extends Component {
  ...
  handleAddRoute = () => {
    ...
    this.props.handleAddRoute(route).then(res => {
      this.props.handleGetUserRoutes(this.state._id).then(res => {
        if (res) {
  ...
  handleUpdateRoute = () => {
    ...
    this.props.handlePageLoader(true);
    this.props.handleUpdateRoute(routeId, route).then(res => {
      this.props.handleGetUserRoutes(this.state._id).then(res => {
        if (res) {
          this.props.handlePageLoader(false);
          this.setState(
  ...
  handleDeleteRoute = () => {
    ...
    this.props.handlePageLoader(true);
    this.props.handleDeleteRoute(routeId).then(res => {
      this.props.handleGetUserRoutes(this.state._id).then(res => {
        if (res) {
          this.props.handlePageLoader(false);
          this.setState(
  ...
  handleSetDeleteDetails = route => {
    const { from, to } = route;
    let routeId = route._id;
    this.setState({ ...this.state, from, to, routeId }, () =>
      this.handleShowDeleteRoute()
    );
  };
  ...
  handleSelect = e => {
    ...
    let vehicleDetails = this.props.vehicleTypes.filter(
  ...
  handleDataRange = e => {
    ...
    return this.setState({ usersPerPage: numberToShow, currentPage: 1 }, () =>
      setTimeout(() => {
        callback();
      }, 100)
    );
  ...
  render() {
    ...
    const currentUsers = this.state.data.slice(
      indexOfFirstUser,
      indexOfLastUser
    );
        ...
        <StyleWrapper>
          <Wrapper className="fixedheight" {...styles.Wrapper}>
            <Column {...styles.Column} isMainSection>
              <Header {...styles.Header}>Add a Route</Header>
              <Form
                inputs={this.state}
