...
class CompanyDashboard extends Component {
    ...
    const verifyFile = files => {
      if (files) {
        ...
        if (currentFileSize > imageMaxSize) {
          swal(
            "File Too Large",
            `This file is not allowed. ${Math.round(
              currentFileSize / 1000000
            )} Mb is too large, maximum allowed size is 10mb`,
            "error"
          );
          return false;
        }
  ...
  handleSubmit = e => {
    ...
    this.setState({ socialMedia, phoneNumber: _phoneNumber }, () =>
  ...
  handleUpdateUser = () => {
    ...
    this.props.handleUpdateUser(_id, user).then(res => {
      if (res) {
        this.setState(
          {
            loading: false,
      ...
      <Layout {...styles.Layout}>
        <Wrapper className="fixedheight" {...styles.Wrapper}>
          <Column {...styles.Column} isMainSection>
            <Header {...styles.Header}>Settings</Header>
            <Form
              inputs={this.state}
